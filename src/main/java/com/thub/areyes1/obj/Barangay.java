/**
 * Class File Name: Barangay.java
 * Author: alvinreyes
 * Date Generate: Jun 7, 2015
 * Description
 */

package com.thub.areyes1.obj;

// TODO: Auto-generated Javadoc
/**
 * The Class Barangay.
 */
public class Barangay {
	
	/** The barangay captain. */
	private String barangayCaptain;

	/**
	 * Gets the barangay captain.
	 *
	 * @return the barangay captain
	 */
	public String getBarangayCaptain() {
		return barangayCaptain;
	}

	/**
	 * Sets the barangay captain.
	 *
	 * @param barangayCaptain the new barangay captain
	 */
	public void setBarangayCaptain(String barangayCaptain) {
		this.barangayCaptain = barangayCaptain;
	}
	
}
